    ...
    <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 text-left">
      ...
      <div className="max-w-xl z-10 text-gray-500">
        ...
        <div className="flex items-center space-x-2 pt-3 pb-1 text-sm">
          ...
          <p className="text-blue-500">
            {courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? "students" : "student"}
          </p>
        </div>

        <p className="text-sm">
          Course By <span className="text-blue-600 underline">Greakstack</span>
        </p>
        ...
        <div className="pt-8 text-gray-800">
          ...
          <div className="pt-5">
            {courseData.courseContent?.map((chapter, index) => (
              <div key={index} className="border border-gray-300 bg-white md:mt-2 rounded">
                <div className="flex items-center justify-between p-3 cursor-pointer select-none" onClick={() => toggleSection(index)}>
                  <div className="flex items-center gap-2">
                    <img className={`transform transition-transform ${openSection[index] ? 'rotate-180' : ''}`} src={assets.down_arrow_icon} alt="arrow_icon" />
                ...
                <div className={`overflow-hidden transition-all duration-500 ${openSection[index] ? 'max-h-96' : 'max-h-0'}`}>
                  <ul className="list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border border-gray-300">
                    {chapter.chapterContent.map((lecture, i) => (
                      <li key={i} className="flex items-center justify-between p-3 cursor-pointer select-none">
...
:<img src={assets.time_left_clock_icon} alt="time_left_clock_icon" className="w-4" />
          ...
          <div className='flex gap-3 items-center pt-3'>
            <p className='text-gary-800 md:text-4xl text-2xl font-semibold'>{currency} {(courseData.coursePrice - courseData.discount * courseData.coursePrice / 100).toFixed(2)}</p>
          ...
          <button className="md:mt-6 mt-4 w-full py-3 round bg-blue-600 text-white font-medium">{isAlreadyenrolled ? 'Already Enrolled' :'Enroll Now'}</button>
